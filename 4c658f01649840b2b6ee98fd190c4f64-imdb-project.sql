...
SELECT 
	(select count(*) from movies where id is NULL) as id_null,
	(select count(*) from movies where title is NULL) as title_null,
	(select count(*) from movies where year is NULL) as year_null,
	(select count(*) from movise where date_published is NULL) as date_published_null,
	(select count(*) from movies where duration is NULL) as duration_null,
	(select count(*) from movies where country is NULL) as country_null,
	(select count(*) from movies where worlwide_gross_income is NULL) as worldwide_gross_income_null,
	(select count(*) from movies where languages is NULL) as laguages_null,
	(select count(*) from movies where production_company is NULL) as production_company_null
;
...
select count(title)as all_movies, month(date_published) as months from movies group by months order by months;
...
#min_avg=1,max_avg=9.8
#min_vote=100,max_vote=999
#min_median=1,max_median=9.5

# 2:Identify the top 10 movies based on average rating.
select title as Movie_Name, avg_rating from movies m
...
#279 movies have 3 median rating.

# 4:Identify the production house that has produced the most number of hit movies (average rating > 8).
select production_company, count(id) as number_of_movies from movies m
...
select m.title,r.avg_rating,g.genre from movies m join ratings r on m.id=r.movie_id 
...
where  r.avg_rating > 8 and genre in(select genre from genre)
...
#mohanlal=8 and mamooty=5 are the top actors

# 4:Identify the top three production houses based on the number of votes received by their movies.
select production_company,sum(total_votes)as votes from movies m join ratings r 
...
when avg_rating < 5 then 'ok_ok' else "ughh" end Category from movies m 
join ratings r on r.movie_id = m.id
